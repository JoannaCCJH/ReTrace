...
def load_config(config_path: str, overrides: list = None) -> dict:
    """Load configuration from YAML file, resolving OmegaConf interpolations.

    Applies dot-notation overrides (e.g. ["model.decoder.num_layers=6"])
    the same way train.py / test_benchmark.py do.
    """
...
def main():
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=[],
        help="Override config values using dot notation (e.g. model.decoder.num_layers=6)"
    )
    ...
    depth_ratio_map = depth_ratio_map.squeeze(1).cpu().numpy()  # [B, H, W]
        ...
        for guidance_scale in guidance_scales:
            for iter_idx in range(args.num_samples_per_scale):
                ...
                pred_traj = create_trajectory_visualization(
                    ...
                    depth_ratio_filled=depth_ratio_filled[img_idx]
                )
                # breakpoint()
                ...
                # rescaled depth
                rescaled_depth = depth_ratio_filled[img_idx] * single_depth[0,0].detach().cpu().numpy()
